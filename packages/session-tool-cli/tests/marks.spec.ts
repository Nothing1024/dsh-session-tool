...
describe('marks argv/shape', () => {
  it('exposes marks list/get without profile boot options', () => {
    ...
    expect(listFlags).toEqual(expect.arrayContaining(['--kind', '--mark', '--prefix', '--format']))
    ...
    const markCmd = session?.commands.find(command => command.name() === 'mark')
    expect(markCmd).toBeDefined()
    const markFlags = (markCmd?.options ?? []).map(option => option.long)
    expect(markFlags).toEqual(expect.arrayContaining(['--add', '--remove', '--format', '--profile']))
  ...
  it('lists and gets jsonl rows without a gateway', async () => {
    ...
    const byMark = await runMain(['marks', 'list', '--mark', 'kind:vibee', '--format', 'json'], home)
    expect(byMark.code).toBe(0)
    expect(JSON.parse(byMark.out)).toEqual([{ id: 's-vibee', tags: ['kind:vibee', 'plan'] }])
    const byPrefix = await runMain(['marks', 'list', '--prefix', 'kind:', '--format', 'json'], home)
    expect(byPrefix.code).toBe(0)
    expect(JSON.parse(byPrefix.out)).toEqual([{ id: 's-vibee', tags: ['kind:vibee', 'plan'] }])
  })
