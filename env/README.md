...
官方 pin：`@deepseek-ai/dsh` / `@deepseek-ai/dsh-base` / `@deepseek-ai/dsh-web-app` 以及仓内每个 `@deepseek-ai/dsh-*` 均为 **0.1.0-rc.7**（`boot.sh` 用 `npx @deepseek-ai/dsh@0.1.0-rc.7`）。会话 tags 是插件标记（`$DSH_HOME/session-tool/marks.jsonl`）：保留名 `kind:vibee` / `kind:delegated` / `kind:hidden` / `ui:aux`；官方 GUI 不显示；后期 Web 用 `listByKind`。
